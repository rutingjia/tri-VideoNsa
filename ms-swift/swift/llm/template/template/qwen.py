...
class Qwen2VLTemplate(Template):
    ...
    def forward_context(self, model, inputs):
        ...
        if self.version == 'v2':
            ...
            from transformers.models.qwen3_vl import modeling_qwen3_vl as modeling_module
...
class Qwen3VLTemplate(Qwen2_5VLTemplate):
    """Dedicated template entry for Qwen3-VL models."""
    version = 'v3'
    required_model_id = 'Qwen/Qwen3-VL-2B-Instruct'

    def _assert_model_id(self, model) -> None:
        model_name = getattr(getattr(model, 'config', None), '_name_or_path', None)
        if model_name != self.required_model_id:
            raise ValueError(
                f'{self.__class__.__name__} only supports `{self.required_model_id}`, '
                f'but got `{model_name}`.')

    def forward_context(self, model, inputs):
        self._assert_model_id(model)
        return super().forward_context(model, inputs)
